--[[
    crc8smbus (aka crc-8, crc-8-ccitt)
    -- generated by `crcany` (https://github.com/madler/crcany)
    -- ported to Lua 5.3 by Andrew Zhilin (https://github.com/zoon)
]]

local byte = string.byte
local type, mtype, assert, print = type, math.type, assert, print

_ENV = nil

-- LuaFormatter off
-- 0-indexed
local table_bytes = {[0]=

}
-- LuaFormatter on

-- @ref
-- uint8_t crc8smbus_byte(uint8_t crc, void const *mem, size_t len) {
--     unsigned char const *data = mem;
--     if (data == NULL)
--         return 0;
--     for (size_t i = 0; i < len; i++) {
--         crc = table_byte[crc ^ data[i]];
--     }
--     return crc;
-- }

local INIT = 0x00
-- Compute the CRC a byte at a time.
-- Returns the CRC of string, applied to the previous CRC value, crc.
-- If string is nil, then the other arguments are ignored, and the initial CRC,
-- i.e. the CRC of zero bytes, is returned
local function crc8smbus_byte(str, crc)
    if str == nil then return INIT end
    crc = crc or INIT
    assert(type(str) == "string")
    assert(crc == (crc & 0xff), "`crc` should be `uint8`")
    for i = 1, #str do crc = table_bytes[crc ~ byte(str, i)] end
    return crc
end

local function from_int(n, crc)
    assert(mtype(n) == "integer")
    assert(n == (n & 0xffffffff), "`n` should be `uint32`")
    crc = crc or INIT
    crc = table_bytes[crc ~ (n >> 24 & 0xff)]
    crc = table_bytes[crc ~ (n >> 16 & 0xff)]
    crc = table_bytes[crc ~ (n >> 8 & 0xff)]
    crc = table_bytes[crc ~ (n & 0xff)]
    return crc
end

-- self-test
local function self_test()

    assert(crc8smbus_byte("123456789") == 0xF4)
    assert(crc8smbus_byte("1") == 0x97)
    assert(crc8smbus_byte("HELLO") == 0x35)
    assert(from_int(0xFABAABBA) == 0x52)
    assert(from_int(0xFF) == 0xF3)

    print("crc8 -- ok")
end
self_test()

-- module
return function(arg, crc)
    if mtype(arg) == "integer" then return from_int(arg, crc) end
    return crc8smbus_byte(arg, crc)
end


